...
rep_p1<-linked %>% 
  distinct(county,event,perc_republican_libertarian) %>% 
  group_by(event) %>% 
  mutate(percentile=ntile(perc_republican_libertarian,4)
  ) %>% 
...
rep_diff<-rep_p1 %>% 
  fixest::feols(perc_republican_libertarian~percentile,
                split = ~event)
pull<-function(x){
...
rep_sum<-rep_p1 %>% 
  ...
  reframe(m=as.character(round(mean(perc_republican_libertarian),2))) %>% 
  pivot_wider(
    names_from = event,
    values_from = m)

colnames(a)<-colnames(rep_sum)

rep_sum<-rbind(rep_sum,
                      a
)
