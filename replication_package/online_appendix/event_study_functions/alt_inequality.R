...
inequality_p1<-linked %>% 
  distinct(county,event,income_inequality) %>% 
  group_by(event) %>% 
  mutate(percentile=ntile(income_inequality,4)
  ) %>% 
...
inequality_diff<-inequality_p1 %>% 
  fixest::feols(income_inequality~percentile,
                split = ~event)
pull<-function(x){
...
a<-lapply(inequality_diff,pull) %>% 
  bind_rows() %>% 
  mutate(first=c("Difference"," ")) %>% 
  relocate(first,.before=1)

inequality_sum<-inequality_p1 %>% 
  group_by(event,percentile) %>% 
  reframe(m=as.character(round(mean(income_inequality),2))) %>% 
  pivot_wider(
    names_from = event,
    values_from = m)

colnames(a)<-colnames(inequality_sum)

inequality_sum<-rbind(inequality_sum,
                  a
)
...
inequality_small_weighted<-finished %>% 
  left_join(top_ten) %>% 
  filter(is.na(inn)) %>% 
  mutate(treat=ifelse(str_detect(percentile,"upper") & event_indicator=="event",1,0)) %>% 
  fixest::feols(ok_per_thousand~treat|date+county,
...
ev_inequality_weighted<-finished %>% 
