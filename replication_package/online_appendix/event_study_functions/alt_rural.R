...
rural_p1<-linked %>% 
  distinct(county,event,perc_pop_rural) %>% 
  group_by(event) %>% 
  mutate(percentile=ntile(perc_pop_rural,4)
  ) %>% 
...
rural_diff<-rural_p1 %>% 
  fixest::feols(perc_pop_rural~percentile,
                split = ~event)
pull<-function(x){
...
rural_sum<-rural_p1 %>% 
  ...
  reframe(m=as.character(round(mean(perc_pop_rural),2))) %>% 
  pivot_wider(
    names_from = event,
    values_from = m)

colnames(a)<-colnames(rural_sum)

rural_sum<-rbind(rural_sum,
               a
)
