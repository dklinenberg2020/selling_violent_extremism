...
all_combos<-expand_grid("county"=unique(fred_inequality$county), 
...
income_p1<-linked %>% 
  distinct(county,event,median_income) %>% 
  group_by(event) %>% 
  mutate(percentile=ntile(median_income,4)
  ) %>% 
...
income_diff<-income_p1 %>% 
  fixest::feols(median_income~percentile,
                split = ~event)
pull<-function(x){
  ...
  starss<-case_when(stars<=.01~"***",
            ...
            stars<=.1~"*",
            TRUE~"")
  rbind(unlist(paste0(scales::dollar(round(x$coeftable[2,1],2)),starss
  )),
  unlist(paste0("(",scales::dollar(round(x$coeftable[2,2],2)),")"))
  )
}

a<-lapply(income_diff,pull) %>% 
  bind_rows() %>% 
  mutate(first=c("Difference"," ")) %>% 
  relocate(first,.before=1)

income_sum<-income_p1 %>% 
  group_by(event,percentile) %>% 
  reframe(m=scales::dollar(mean(median_income))) %>% 
  pivot_wider(
              names_from = event,
              values_from = m)

colnames(a)<-colnames(income_sum)

income_sum<-rbind(income_sum,
      a
)
...
income_small_weighted<-finished %>% 
  left_join(top_ten) %>% 
  filter(is.na(inn)) %>% 
  mutate(treat=ifelse(str_detect(percentile,"upper") & event_indicator=="event",1,0)) %>% 
  fixest::feols(ok_per_thousand~treat|date+county,
...
rm(list=ls()[-which(str_detect(ls(),".*weighted|.*sum"))])
