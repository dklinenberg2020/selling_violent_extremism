...
source(here::here("online_appendix","model_functions", "00_sc_functions.R"))
...
for(i in 1:nrow(label_dat)){
  assign(label_dat$am_element[i],
         sc_cleaner(word=gsub( " .*$", "", label_dat$sale[i]) ,
                  pre_period_length  =  14,
                  placebo_pre_length = 7,
                  w.constr=list(name = "simplex", Q = 1, Q2=NULL,constant=T),
                  placebo = T),
